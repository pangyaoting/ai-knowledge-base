...
> 🌐 线上演示：**https://aiknowbase.cn**（腾讯云 · Nginx + HTTPS · GitHub Actions 自动部署 · 已完成 ICP 备案）
...
| 数据看板 | 使用统计、Token 消耗（含研究报告/自主研究按模型归因）、近 7 日趋势（ECharts） | ✅ 完成 |
| 部署上线 | 腾讯云 + Nginx + HTTPS（certbot）+ CI/CD 自动部署 + 域名备案 | ✅ 已上线 |
| 合规 | 备案号页脚展示、AI 生成内容显式标识、隐私政策/用户协议 | ✅ 完成 |
| 2FA | TOTP 双因素认证 | 🚧 规划中 |
