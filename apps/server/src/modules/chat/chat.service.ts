...
export class ChatService {
  ...
  private buildPrompt(
    ...
    // 代码解析格式规范：用户要求"解析/讲解代码"时，用 行号|代码|说明 三列表格，
    // 避免"代码一段配一句话"的碎片式排版（可读性差、重点不突出）。
    // 探测问题意图：点名了代码文件名（xxx.vue/.ts/…）或含"解析/逐行/讲解/每一行"等词。
    ...
    if (wantsCodeWalkthrough) {
      systemParts.push(
        '当用户要求解析代码时：请用 **三列表格** 组织讲解——列依次为 `行号 | 代码 | 说明`。',
        '规则：① 先按逻辑分组给出需要讲解的完整代码块（可多组），再对每组紧跟一张三列表格，逐行说明该行作用；',
        '② 代码单元格保留原代码（含缩进），行号为该代码在文件中的真实行号（若资料未给出则用组内序号并注明）；',
        '③ 说明单元格用一句话讲清该行意图（做了什么/为什么），避免整段空话；',
        '④ 简单到无需解释的行（如 `}` 收尾）可跳过不列表，但请在表格后注明"其余为结构收尾，略"；',
        '⑤ 最后给一段 2~3 句的逻辑总结，讲清这几个分组如何串成完整功能。',
        '不要使用"代码块 + 单句解释"的逐行穿插排版。',
      );
    }
