...
| 09-12 | 服务器加固：日志留存 60 天（pm2-logrotate + logrotate）、`pm2 startup` 开机自启 | ✅ 见 §八⑦⑧ |
| 09-12 | **公安联网备案**提交（beian.mps.gov.cn，主体=个人，服务器所在地=广东省广州市） | ✅ **通过：粤公网安备44098202441209号** |
| 通过后 | 备案号填进 `POLICE_BEIAN` → 首页页脚与 ICP 备案号并列展示（可点，跳公安查询页） | ✅ |
...
### 5.3 收尾（代码侧已完成，见 §八）
- ✅ 备案号在**首页页脚中央**展示并链接 `beian.miit.gov.cn`（法条只要求"主页底部"，故不铺到每个内页；全站页脚保留合规页入口 + AI 提示）
...
| # | 要求 | 依据 | 落实 |
|---|---|---|---|
| ① | **主页底部中央标明备案编号，并链接工信部备案系统** | 《非经营性互联网信息服务备案管理办法》第 13 条（原文只写"**主页**底部"） | ✅ **备案号仅挂在首页页脚**（`Layout.vue` 里 `isHome` 判断）；全站页脚保留「隐私政策 / 用户协议」入口与 AI 提示 —— 法规是下限，深链进内页的用户也能找到合规文本 |
| ② | **公安联网备案**（上线后 30 日内） | 公安部 33 号令第 11/12 条 | ✅ **已通过**：`粤公网安备44098202441209号`，与 ICP 备案号并列展示在首页页脚（`POLICE_BEIAN`） |
...
| ⑦ | **日志留存 ≥ 60 日** | 《互联网安全保护技术措施规定》（公安部 82 号令）第 8 条：记录并留存用户登录、使用信息不少于 60 日 | ✅ 2026-09-12 已配：`pm2-logrotate`（retain 60 / max_size 10M / compress）+ nginx `logrotate` 由 `rotate 14` 改 `rotate 60`；服务器上查证：`pm2 conf pm2-logrotate`、`grep rotate /etc/logrotate.d/nginx` |
| ⑧ | **服务开机自启** | 备案要求站点真实可用 | ✅ `pm2-root.service` enabled（`pm2 startup systemd -u root --hp /root` + `pm2 save`）—— 服务器重启后 `kb-server` 自动恢复，否则重启即 502 |
...
浏览器侧（只有真人能验）待确认清单：锁头正常 → **首页**页脚备案号可点 → 对话逐字流式 + `AI 生成` 标识 → 无痕窗口能开隐私政策 → 换头像即时生效。
